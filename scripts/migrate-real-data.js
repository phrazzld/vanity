...
async function migrateRealData() {
  ...
  try {
    ...
    const readingsCurrentlyMatch = readingsContent.match(
      /const READINGS_CURRENTLY: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readingsCurrentlyMatch && readingsCurrentlyMatch[1]) {
      readingsArrays.push(readingsCurrentlyMatch[1]);
    }

    const readings2025Match = readingsContent.match(
      /const READINGS_2025: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readings2025Match && readings2025Match[1]) {
      readingsArrays.push(readings2025Match[1]);
    }

    const readings2024Match = readingsContent.match(
      /const READINGS_2024: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readings2024Match && readings2024Match[1]) {
      readingsArrays.push(readings2024Match[1]);
    }

    const readings2023Match = readingsContent.match(
      /const READINGS_2023: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readings2023Match && readings2023Match[1]) {
      readingsArrays.push(readings2023Match[1]);
    }

    const readings2022Match = readingsContent.match(
      /const READINGS_2022: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readings2022Match && readings2022Match[1]) {
      readingsArrays.push(readings2022Match[1]);
    }

    const readings2021Match = readingsContent.match(
      /const READINGS_2021: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readings2021Match && readings2021Match[1]) {
      readingsArrays.push(readings2021Match[1]);
    }

    const readings2020Match = readingsContent.match(
      /const READINGS_2020: Reading\[\] = (\[[\s\S]*?\]);/
    );
    if (readings2020Match && readings2020Match[1]) {
      readingsArrays.push(readings2020Match[1]);
    }

    const readingsDroppedMatch = readingsContent.match(
      /const READINGS_DROPPED: Reading\[\] = (\[[\s\S]*?\]);/
    );
    ...
    const readingsMatch = readingsContent.match(
      /export const READINGS: Reading\[\] = (\[[\s\S]*\]);/
    );
    ...
    function parseArrayString(arrayString) {
      ...
      const isoDateReplacedString = arrayString.replace(
        /new Date\(['"](.*?)['"]\)/g,
        (match, dateStr) => {
