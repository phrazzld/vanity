...
export interface ListStateOptions<TItem, TFilter extends Record<string, any> = Record<string, any>> {
  /** Function to fetch data */
  fetchItems: (params: any) => Promise<{ data: TItem[]; totalCount: number; currentPage: number; totalPages: number; pageSize: number }>;
...
function createListReducer<TItem, TFilter extends Record<string, any> = Record<string, any>>() {
  return (state: ListState<TItem, TFilter>, action: ListAction<TItem, TFilter>): ListState<TItem, TFilter> => {
    switch (action.type) {
      case 'SET_ITEMS':
